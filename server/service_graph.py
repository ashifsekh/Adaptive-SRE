...
class ServiceGraph:
    ...
    def propagate(self, dt: float = 1.0) -> None:
        prev_health = {name: svc.health for name, svc in self.services.items()}
        for upstream_name, downstream_deps in DEPENDENCY_GRAPH.items():
            ...
            for downstream_name, weight in downstream_deps.items():
                ...
                # Base bleed from upstream degradation + momentum from recent upstream drop.
                health_degradation = max(0, (1.0 - upstream.health)) * weight * dt * 0.093
                upstream_drop = max(0.0, prev_health.get(upstream_name, upstream.health) - upstream.health)
                health_degradation += upstream_drop * weight * dt * 0.38
                ...
                # First observed degradation gets a deterministic causal onset signature.
                if downstream.onset_timestamp is None and (downstream.health < 0.95 or downstream.error_rate > 0.05):
                    onset_offsets = {
                        "auth": 2.3,
                        "payment": 4.1,
                        "cache": 1.6,
                    ...
                    offset = onset_offsets.get(downstream_name, 1.0)
                    downstream.onset_timestamp = time.time() - offset
    ...
    def apply_fault(self, service_name: str, fault_type: str) -> None:
        ...
        fault_configs = {
            ...
            "connection_exhaustion": {"health": 0.15, "error_rate": 0.9, "latency_ms": 2200, "cpu_pct": 90}
        }
